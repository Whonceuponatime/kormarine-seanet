...
class NetworkDiagram {
    ...
    startDrag(e, element) {
        ...
        if (match) {
            ...
            // Convert mouse coordinates to SVG coordinates
                    const svgX = ((e.clientX - svgRect.left) / svgRect.width) * 840;
        const svgY = ((e.clientY - svgRect.top) / svgRect.height) * 420;
    ...
    drag(e) {
        ...
        const svgX = ((e.clientX - rect.left) / rect.width) * 840;
        const svgY = ((e.clientY - rect.top) / rect.height) * 420;
        
        // Calculate new position
        const newX = Math.max(0, Math.min(780, svgX - this.dragOffset.x));
        const newY = Math.max(0, Math.min(350, svgY - this.dragOffset.y));
